...
contract("Testing Allowance and Approval Process + Increase and Decrease Approval. ",function(accounts){
  ...
  describe("Make allowance from accounts[1] as owner. Make transfer as accounts[1] to accounts[2] and check if it works.",function(){
    ...
    it("Accounts[1] transfer 50 tokens from accounts[0] to accounts[2] suceed.",function(){
      expect(viddoContract.transferFrom(accounts[0],accounts[2],50,{"from":accounts[1]})).to.be.eventually.fulfilled;
    });

    it("Accounts[1] transfer 100 tokens from accounts[0] to accounts[2] fails. Not enough tokens approved.",function(){
      expect(viddoContract.transferFrom(accounts[0],accounts[2],100,{"from":accounts[1]})).to.be.eventually.rejected;
    });

    it("Accounts[3] transfer 50 tokens from accounts[0] to accounts[2] fails.",function(){
      expect(viddoContract.transferFrom(accounts[0],accounts[2],50,{"from":accounts[3]})).to.be.eventually.rejected;
    });
    ...
    it("Accounts[1] transfer 50 tokens from accounts[0] to accounts[1] succeed.",function(){
      expect(viddoContract.transferFrom(accounts[0],accounts[1],50,{"from":accounts[1]})).to.be.eventually.fulfilled;
    });
